  ...
  const loadKeys = async () => {
    ...
    if (selected && typeof selected === "string") {
      try {
        ...
        if (data.SLOPSMITH_SNG_PC_KEY) setSngKey(data.SLOPSMITH_SNG_PC_KEY);
      } catch (e) {
  ...
  const loadNewKeys = async () => {
    ...
    if (selected && typeof selected === "string") {
      try {
        ...
        if (data.SLOPSMITH_SNG_PC_KEY) setNewSngKey(data.SLOPSMITH_SNG_PC_KEY);
      } catch (e) {
        alert("Failed to load keys: " + e);
      }
    }
  ...
  const saveNewKeys = async () => {
    ...
    if (selected) {
      const data = {
        ...
        SLOPSMITH_SNG_PC_KEY: newSngKey,
      };
      try {
        await invoke("write_file_contents", {
          path: selected,
        ...
        <div className="sidebar-keys">
          ...
          <div className="key-input">
            ...
            <input type="text" value={sngKey} onChange={e => setSngKey(e.target.value)}
                   placeholder="SLOPSMITH_SNG_PC_KEY..." />
          </div>
        </div>
      </aside>
